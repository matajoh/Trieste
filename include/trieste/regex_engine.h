...
namespace trieste::regex
{
  ...
  struct RuneClass
  {
    ...
    {
      if (gap_lo <= 0xD7FF && gap_hi >= 0xD800)
      {
        if (gap_lo <= 0xD7FF)
      ...
      else if (gap_lo >= 0xD800 && gap_lo <= 0xDFFF)
      {
        if (gap_hi >= 0xE000)
          out.push_back({0xE000, gap_hi});
      }
  ...
  public:
    ...
    private:
      ...
      // Bitset dedup for non-capturing paths with ≤128 NFA states.
  ...
  private:
      ...
      bool prev_is_word) const
    {
      ...
      else
      {
        ...
        start_list(
          current_states, start_state_, epoch, ctx, false, false, false);
        ...
        while (has_current_rune && !current_states.empty())
        {
          ...
          step(
            current_states, rune_value, next_states, epoch, ctx, false, false);
          std::swap(current_states, next_states);
          if (is_match(current_states))
            best = pos;
        }
      ...
      bool at_end = false) const
    {
      ...
      while (!stack.empty())
      {
        ...
        if (s->label == Split)
        {
          // Push next_alt first so next is popped first (greedy DFS).
    ...
    closure_flags(bool boundary_match, bool at_start, bool at_end)
    {
      return static_cast<uint8_t>(
        (boundary_match ? 1 : 0) | (at_start ? 2 : 0) | (at_end ? 4 : 0));
    }
    ...
    struct Frag
    {
      ...
      std::list<State*> dangling;
    };

    static inline std::list<State*>&
    append(std::list<State*>& lhs, std::list<State*>& rhs)
    {
      lhs.splice(lhs.end(), rhs);
      return lhs;
    }

    static inline void patch(std::list<State*>& targets, State target)
    {
    ...
    // Supports 1-char (L, N, ...) and 2-char (Lu, Nd, ...) category names
    // from Unicode 15.1. \P{} is the negation of \p{}.
    // `pos` is positioned after the 'p'/'P'; on return it points past '}'.
      ...
      const std::string_view& pattern, size_t& pos, bool negate)
    {
      ...
      std::string_view cat_name(pattern.data() + name_start, pos - name_start);
      pos++; // skip '}'
      ...
      // Ranges are pre-sorted from unicode data, but finalize() for
    ...
    {
      return syntax_mode_ == SyntaxMode::Extended;
    }

    bool is_strict_iregexp_mode() const
    {
      return syntax_mode_ == SyntaxMode::IregexpStrict;
    }

    // Centralized syntax-policy helpers used by parser gating.
    // Step-3 wiring: helpers are introduced first; parse-site enforcement is
    // applied in the next step to keep diffs small and behavior transitions
    // easy to validate.
    ...
    {
      return pos < pattern.size() && pattern[pos] >= '0' && pattern[pos] <= '9';
    }
    ...
    // Parse a POSIX character class like [:alpha:] inside a bracket expression.
    // `pos` is positioned after the '[:', on return it points past ':]'.
    ...
    // `pos` is positioned after the opening '['; on return it points past ']'.
    rune_t parse_char_class(const std::string_view& pattern, size_t& pos)
    {
      ...
      if (pos >= pattern.size() || pattern[pos] == ']')
      {
        if (negated && pos < pattern.size() && pattern[pos] == ']')
        {
          set_error(ErrorCode::ErrorMissingBracket);
          return RuneClassBase;
        }
        // Empty class [] — also malformed.
        set_error(ErrorCode::ErrorMissingBracket);
        return RuneClassBase;
      }
      ...
      while (pos < pattern.size() && pattern[pos] != ']')
      {
        ...
        if (pattern[pos] == '\\')
        {
          ...
          if (esc_rune.value == 'p' || esc_rune.value == 'P')
          {
            ...
            rc.merge(rune_classes_[class_ref_index(ref)]);
            continue;
          }
    ...
    // Syntax-policy matrix for Extended vs Strict iregexp modes is in
    // notes/regex-syntax-policy.md.
    ...
    std::vector<rune_t> regexp_to_postfix_runes(const std::string_view& pattern)
    {
      ...
      // Helper: record that an atom begins at current postfix position.
      auto begin_atom = [&]() {
      ...
      while (pos < pattern.size())
      {
        ...
        if (escaped)
        {
          ...
          if (rune.value == 'p' || rune.value == 'P')
          {
            bool negate = (rune.value == 'P');
          ...
          {
            rune_t lower = rune.value | 0x20; // tolower for ASCII
            RuneClass rc;
          ...
          if (rune.value == 'b')
          {
            ...
            if (need_concat)
              push_concat();
            begin_atom();
            postfix.push_back(WordBoundary);
            need_concat = true;
            continue;
          }
          ...
          switch (rune.value)
          {
            ...
            case ',':
              ...
              break;
            default:
              if (allow_extended_identity_escape(rune.value))
              {
                literal = rune.value;
                break;
              }
        ...
        switch (rune.value)
        {
          ...
          case '$':
            if (need_concat)
              push_concat();
            begin_atom();
            postfix.push_back(EndAnchor);
            need_concat = true;
            break;

          case ']':
            // RE2-compatible literal ']' in standalone position.
            if (!need_concat && allow_standalone_closer_literals())
            {
              begin_atom();
              postfix.push_back(']');
              need_concat = true;
            ...
            // Bare ']' after an atom is malformed.
            set_error(ErrorCode::ErrorStrictSyntax);
            return {};
          ...
          case '{':
          {
            ...
            if (
              need_concat && allow_standalone_closer_literals() &&
            ...
            if (n == 0 && !unbounded && m == 0)
            {
              ...
              if (
                need_concat_before_atom && !operator_stack.empty() &&
          ...
          case LParen:
          {
            ...
            if (capturing)
            {
              cap_id = num_captures_++;
              if (num_captures_ > MaxCaptures)
              {
          ...
          case RParen:
            ...
            if (operator_stack.empty() || group_start_stack.empty())
            {
              // Unmatched ')' — malformed regexp.
          ...
          case Asterisk:
            if (!need_concat || atom_is_quantified)
            {
              set_error(ErrorCode::ErrorRepeatOp);
              return {};
            }
            parse_quantifier_suffix(
              ZeroOrMore,
              LazyZeroOrMore,
              postfix,
              atom_is_quantified,
      ...
      if (escaped)
      {
        // Trailing backslash — malformed per RFC 9485.
        set_error(ErrorCode::ErrorTrailingBackslash);
        return {};
      }
    ...
    build_quantifier(std::vector<Frag>& stack, QuantifierKind kind, bool lazy)
    {
      ...
      switch (kind)
      {
        case QuantifierKind::QZeroOrOne:
          ...
          operand.dangling.push_back(skip_ptr());
        ...
        case QuantifierKind::QZeroOrMore:
          ...
          stack.push_back({state, {skip_ptr()}});
          break;
        ...
        case QuantifierKind::QOneOrMore:
          if (lazy)
            ...
            state = create_state(Split, operand.start);
          patch(operand.dangling, state);
          stack.push_back({operand.start, {skip_ptr()}});
          break;
      }
    }
    ...
    {
      size_t idx = rune_classes_.size();
      if (idx > (RuneClassMax - RuneClassBase))
      {
        set_error(ErrorCode::ErrorPatternTooLarge);
        return RuneClassBase;
      }
      rune_classes_.push_back(rc);
